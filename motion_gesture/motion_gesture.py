...
import motion_gesture
from motion_gesture.direction import Direction
if os_name == "nt":
    from motion_gesture.nt_window_action import WindowAction
else:
    from motion_gesture.kde_window_action import WindowAction
...
ap.add_argument("-m", "--min-sample", type=int, default=5, help="minimum sample")
args = vars(ap.parse_args())
camera = cv2.VideoCapture(args["file"])

logging.basicConfig(level=args["log"])

logging.info("Loading {}".format(path_dirname(motion_gesture.__file__) + "haarcascade_frontalface_default.xml"))
face_cascade = cv2.CascadeClassifier(path_dirname(motion_gesture.__file__) + "/haarcascade_frontalface_default.xml")
